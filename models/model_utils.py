...
import numpy as np
from models.adder import Adder2D, adder_func
...
#COLLAPSED Linear block + Adder
class LinearBlock_c_adder(tf.keras.layers.Layer):
    def __init__(self,
                 in_filters: int,
                 num_inner_layers: int,
                 kernel_size: int,
        ...
        --------------------------------------------------------------------------
                                            |
                    input_tensor -> Actual convkxk(inCh, outCh)     (:Replaced by Adder)    
                                            |
                                        Final output
        """
        def conv2d(filters: int, kernel_size_: int, padding_: str) -> tf.keras.layers.Layer:
        ...
        # def conv3x3(in_planes, out_planes, stride=1) -> tf.keras.layers.Layer:
        #     " 3x3 convolution with padding "
        #     return adder.Adder2D(in_planes, out_planes, kernel_size=3, stride=stride, padding=1, bias=False)
        
        # def conv1x1(in_planes, out_planes, stride=1) -> tf.keras.layers.Layer:
        #     """1x1 convolution"""
        ...
        ### Replace with Adder
        # conv1 = conv1x1(in_filters, feature_size)
        # conv2 = conv3x3(in_filters, feature_size) if kernel_size==3 else conv2d(feature_size, kernel_size, "valid")
        # conv3 = conv1x1(feature_size, out_filters) if kernel_size==3 else conv2d(out_filters, [1, 1], "valid")

        # self.out_adder = tf.keras.Sequential([Adder2D(self.in_filters, self.out_filters, kernel_size=3, padding=1, trainable=False)])
    ...
    def call(self, inputs):
        ...
        if self.kx != 3:
            ...
            out = adder_func(inputs, wt_tensor)
            # out = self.out_adder([inputs, wt_tensor])
